    ...
    public void run(String... args) {
        ...
        if (branchRepository.count() == 0) {
            ...
            branchRepository.saveAll(List.of(
                    BankBranch.builder().bankName("Maverick Bank").branchName("Main Branch").ifscCode("MAVK0000001").address("123 Maverick St").city("New York").state("NY").build(),
                    BankBranch.builder().bankName("Maverick Bank").branchName("Downtown").ifscCode("MAVK0000002").address("456 Downtown Ave").city("New York").state("NY").build(),
                    BankBranch.builder().bankName("State Bank of India").branchName("Mumbai Main").ifscCode("SBIN0000001").address("Nariman Point").city("Mumbai").state("MH").build(),
                    BankBranch.builder().bankName("HDFC Bank").branchName("Delhi Connaught Place").ifscCode("HDFC0000001").address("CP").city("Delhi").state("DL").build()
            ));
        ...
        if (loanProductRepository.count() == 0) {
            ...
            loanProductRepository.saveAll(List.of(
                    LoanProduct.builder().loanName("Standard Personal Loan").loanType(LoanType.PERSONAL)
                            .minimumAmount(new BigDecimal("10000")).maximumAmount(new BigDecimal("500000")).interestRate(new BigDecimal("12.5"))
                            .minTenureMonths(6).maxTenureMonths(60).description("Flexible personal loan for all your needs").isActive(true).build(),
                    LoanProduct.builder().loanName("Dream Home Loan").loanType(LoanType.HOME)
                            .minimumAmount(new BigDecimal("500000")).maximumAmount(new BigDecimal("20000000")).interestRate(new BigDecimal("8.5"))
                            .minTenureMonths(60).maxTenureMonths(360).description("Affordable home loans with long tenures").isActive(true).build(),
                    LoanProduct.builder().loanName("Drive Away Car Loan").loanType(LoanType.VEHICLE)
                            .minimumAmount(new BigDecimal("100000")).maximumAmount(new BigDecimal("3000000")).interestRate(new BigDecimal("9.5"))
                            .minTenureMonths(12).maxTenureMonths(84).description("Quick vehicle financing").isActive(true).build(),
                    LoanProduct.builder().loanName("Startup Business Loan").loanType(LoanType.BUSINESS)
                            .minimumAmount(new BigDecimal("50000")).maximumAmount(new BigDecimal("10000000")).interestRate(new BigDecimal("14.0"))
                            .minTenureMonths(12).maxTenureMonths(120).description("Empower your business growth").isActive(true).build()
            ));
